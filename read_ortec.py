...
def parse_ortec_chn_tail(ss):
    ...
    assert (len(ss) >= 512), "Bad tail record"
    ...
    cali = {"eng_zero_intercept": d_cali[0], "eng_slope": d_cali[1],
            "peak_shape_zero_intercept": d_cali[3],
            "peak_shape_slope": d_cali[4]}
    if s_type == -102:
        cali["eng_quadratic_term"] = d_cali[3]
        cali["peak_shape_quadratic_term"] = d_cali[5]

    # Descriptions
    d_desc = unpack("<1b 63s 1b 63s", ss[256:384])
    desc = {"detector": [d_desc[0], c_char_p(d_desc[1]).value.decode()],
            "sample": [d_desc[2], c_char_p(d_desc[3]).value.decode()]}

    # for Old AlphaVision version
    if s_type == -101:
        chn_type = "Early Versions"
    else:
        if s_type == -102 and unpack("<1I", ss[384:388]) == 0x53495641:
            chn_type = "Old AlphaVision version"
            # ss[388:512], the Old AlphaVision info
            desc["rAlphaVision"] = unpack("<32s 10s 10s 3f1h7f 30s", ss[388:])
        else:
            chn_type = "New Versions"
...
def parse_ortec_chn(fin):
    """Parse ORTEC integer data spectrum file (*.chn).
    ...
    fmt_d = "<{}i".format(head["channel_length"])
...
def parse_ortec_spc_header(ss):
    ...
    assert (len(ss) >= 128), "Bad 1st record"

    # File type and flags
    r_types = unpack("<3h2x", ss[:8])
    assert r_types[0] == 1
    spc_type, spc_type_desc, is_real = {
        1: (1, "Integer Spectrum", False),
        5: (2, "Real Spectrum", True),
        7: (3, "Real Net Spectrum", True),
        3: (3, "Integer Net Spectrum", False)
