...
class WDParser():
    ...
    def __init__(self,
        ...
        self.__dType = dType  # array type: 'H' -- 2-byte; 'B' -- 1-byte
        self.__compressType = compressType
        fopen_type = "r" if self.__isASCII else "rb"
        if compressType != None:
            if compressType == "bz2":
                self.__fdata = bz2.BZ2File(fndata, fopen_type)
            elif compressType == "gz":
                self.__fdata = gzip.GzipFile(fndata, fopen_type)
            else:
                raise Exception("Un-supported compressType!")
        else:
            self.__fdata = open(fndata, fopen_type)
    ...
    def get_one_event_text(self):
        ...
        if "Pattern" in hdict: hlist[2] = hdict["Pattern"]
        if "Channel" in hdict: hlist[3] = hdict["Channel"]
        if "Event Number" in hdict: hlist[4] = hdict["Event Number"]
        if "Trigger Time Stamp" in hdict:
            hlist[5] = hdict["Trigger Time Stamp"]
